...
export async function isValidChecksum(data: ArrayBuffer, checksum: string): Promise<boolean> {
    const calculatedChecksum = await getChunkChecksum(data);
    console.log(`Calculated checksum: ${calculatedChecksum}, Expected: ${checksum}`);
